...
(define (list-partitions total max-pieces max-value)
  ; *** YOUR CODE HERE ***
        (define (partitions accumulated total max-pieces max-value)
                (cond ((= total 0) (list accumulated))
                        ((or (< total 0) (= max-pieces 0) (< max-value 0)) '())
                        (else (append (partitions (append accumulated (list max-value)) (- total max-value) (- max-pieces 1) max-value)
                              (partitions accumulated total max-pieces (- max-value 1))))))
...
(define (tree-sums tree)
  ...
  (cond
    ...
    ((list? (children tree)) 
      (define (sum_path x)
        (+ (entry tree) x))
    (map sum_path (accumulate append (list) (map tree-sums (children tree)))))))
