...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        proc = procedure.env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, proc)
        

    elif isinstance(procedure, MuProcedure):
        "*** YOUR CODE HERE ***"
        proc = env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, proc)
    else:
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if not scheme_symbolp(target.first):
            raise SchemeError
        rest = vals.second
        args = Pair(target.second, rest)
        env.define(target.first, do_lambda_form(args, env))
        return target.first
...
def scheme_optimized_eval(expr, env):
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
            ...
            args = rest.map(lambda oper: scheme_optimized_eval(oper, env))
